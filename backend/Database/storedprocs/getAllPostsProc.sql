CREATE OR ALTER PROCEDURE getAllPostsProc(@userid Varchar(255))
    AS
    BEGIN
        SELECT p.postid, p.userid, u.username, u.profilepic, p.postwords, p.createdat, (select userid from postlikes where userid = @userid and postid = p.postid) as curuserliked, COUNT(l.likeid) AS likes, COUNT(m.postid) AS comments, p.postpic
        FROM posts p
        ...
        GROUP BY p.createdat, p.postid, p.userid, u.username, u.profilepic, p.postwords, p.postpic
